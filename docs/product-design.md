...
- 每个 Worktree 一个内嵌终端。
- 删除 = `git worktree remove`（去掉目录）；不自动删
  本地分支。
- 用户用裸 git 建的 worktree，添加 Project 时可导入。
...
创建 / 列出 / 删除 / 导入 worktree；每树一个通用
终端；只读 diff；在 Cursor / 访达打开；重启后恢复
侧栏。
...
产品文档已放入本仓库 `docs/`。规格写完后，实现计划
另开，仍不在本文展开。拍板前的「请选字母」流程已
结束。代码只写在本仓库
